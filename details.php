...
if($act=="add"){
    if($usid==-1){
        ...
        $cartquery=mysqli_query($conn,"select * from cart where uid=$usid and siid=$subid and checked=0");
        while($cartrow=mysqli_fetch_assoc($cartquery)){
            $iscart=1;
        }
        if($iscart==0){
            mysqli_query($conn,"insert into cart (siid,uid,quantity) values ($subid,$usid,1)");
            header("location:errororsucc.php?reason=addcart&succ=1&text=去购物车&back=cart.php&usid=$usid&veri=$veri&usr=$usr");
            die;
        }elseif($iscart==1){
            mysqli_query($conn,"start transaction");
            $quantityquery=mysqli_query($conn,"select quantity from cart where uid=$usid and siid=$subid for update");
            while ($quantityrow=mysqli_fetch_row($quantityquery)){
                $quantity=$quantityrow[0];
            }
            $quantity=$quantity+1;
            mysqli_query($conn,"update cart set quantity=$quantity where uid=$usid and siid=$subid");
...
<div class="container">
    ...
    <div class="main">
        <div class="part1">
            <div class="picbar">
                <?php
                $picquery1=mysqli_query($conn,"select subitemtopic.pic from subitems,subitemtopic where subitemtopic.siid=subitems.id and subitems.id=$subid");
            ...
            if($subnum!=1){
                ...
                $subquery=mysqli_query($conn,"select subitems.id from subitems,items where subitems.iid=items.id and items.id=$item");
                while ($subrow=mysqli_fetch_row($subquery)){
                    $allsubitemid=$subrow[0];
                    $allpicquery=mysqli_query($conn,"select pic from subitemtopic where siid=$allsubitemid limit 1");
        ...
        <div class="line"></div>
        <?php
            $commentnumquery=mysqli_query($conn,"select count(*) from comments,items,subitems where items.id=subitems.iid and subitems.id=comments.siid and items.id=$item");
            ...
            if($commentnum==0){
                ...
                $commentquery=mysqli_query($conn,"select subitems.subname,users.username,comments.text,comments.year,comments.month,comments.day from comments,users,subitems,items where comments.uid=users.id and comments.siid=subitems.id and subitems.iid=items.id and items.id=$item limit 2");
        ...
        <div class="itemdetail">
            ...
            <?php
            $picquery2=mysqli_query($conn,"select pic from subitemtopic where subitemtopic.siid=$subid");
        ...
        <div class="iteminfo">
            ...
            <div class="body">
                <ul>
                    <?php
                    $infoquery=mysqli_query($conn,"select text from subitemtotext where siid=$subid");
        ...
        <div class="tips">
            ...
            <?php
            $tipquery=mysqli_query($conn,"select id,text from tips");
            while ($tiprow=mysqli_fetch_row($tipquery)){
                $tipid=$tiprow[0];
                $tiptext=$tiprow[1];
                echo "<div class='body'><h1>$tiptext</h1>";
                echo "<ul>";
                $subtipquery=mysqli_query($conn,"select text from subtips where tid=$tipid");
