...
if($usrqry==1 && $usr==$realname && $veri==$realver && $role=='1'){
    ...
    if($chosen==1){
        ...
        while ($orderrow=mysqli_fetch_row($orderquery)) {
            ...
            if($orderstatus==1 || $orderstatus==2) {
                echo "<div class='cancelbutton'";if($opt==-1){echo " onclick=\"location.href='rootmanage.php?usid=$usid&usr=$usr&veri=$veri&chosen=$chosen&opt=ccancel&optid=$orderid'\"";}echo ">强制取消订单</div>";
            }
            if($orderstatus==2) {
